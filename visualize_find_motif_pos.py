#### ::: Modified from DNABERT-viz to find motifs enriched in positive samples::: ####
...
def main():
    ...
    if args.data_name=='all':
        ...
        if args.vis_task=='attr':
            att_scores = np.load(os.path.join(args.npy_dir, "attributions_" + args.data_name + ".npy"))
    ...
    # run motif analysis
    merged_motif_seqs = utils.motif_analysis(pos_seq,
                                        neg_seq,
                                        pos_att_scores,
    ...
    pl.dump(merged_motif_seqs,open(args.save_file_dir+'/motif_seqs_dict_pos.plk','wb'))
